...
import type { Candle, CandlesByTimeframe, MarketChartBundle, Timeframe } from "../../../../../lib/market/types";
...
const twelveDataTimeoutMs = 10000;
...
async function fetchTwelveData<T extends { status?: string; code?: number; message?: string }>(path: string, parameters: Record<string, string>) {
  ...
  try {
    response = await fetch(endpoint, {
      ...
      signal: AbortSignal.timeout(twelveDataTimeoutMs),
    });
  } catch (error) {
    if (error instanceof DOMException && error.name === "TimeoutError") {
      throw new TwelveDataError("Twelve Data hat für diese Zeitreihe zu lange gebraucht.", 504);
    }
    throw new TwelveDataError("Twelve Data ist momentan nicht erreichbar.", 502);
  }
  let payload: T;
  try {
...
export async function GET(request: Request) {
  const symbol = normalizeTicker(new URL(request.url).searchParams.get("symbol") ?? "");
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Marktdaten konnten nicht vom Provider geladen werden.";
    return NextResponse.json({ error: message }, { status: getMarketDataErrorStatus(error) });
  }
}

function getMarketDataErrorStatus(error: unknown) {
